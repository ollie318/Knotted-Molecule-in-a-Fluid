...
POSITION updateLast(POSITION nMinusOnePos, POSITION nPosOld, POSITION nPosPlusOne, POSITION nPosNew, CONSTANTS c);
...
int main()
{
    ...
    c.Q_0 = 236.34E-7;                                                  //Average bond length
    ...
    for(loopcount = 0; loopcount < noOfRuns; loopcount++){
        ...
        for(i_dash = 1; i_dash < N-1; i_dash ++){
        ...
        PositionArrayNew[N-1] = updateLast(PositionArrayOld[N-2], PositionArrayOld[N-1], PositionArrayOld[N-6], PositionArrayNew[N-1], c);
...
POSITION update(POSITION nMinusOnePos, POSITION nPosOld, POSITION nPosPlusOne, POSITION nPosNew, CONSTANTS c){
    ...
    nPosNew.xPos = nPosOld.xPos + (c.h*( FENEForces.FENE_x1 - FENEForces.FENE_x2 + BrownianForces.BrownianForce_x)/StokeDragForce);            //x INCREASES by FENE acc (a = F/m) and Brownian acc

    nPosNew.yPos = nPosOld.yPos + (c.h*( FENEForces.FENE_y1 - FENEForces.FENE_y2 + BrownianForces.BrownianForce_y)/StokeDragForce);

    nPosNew.zPos = nPosOld.zPos + (c.h*( FENEForces.FENE_z1 - FENEForces.FENE_z2 + BrownianForces.BrownianForce_z)/StokeDragForce);

    printf("%.12lf\n", (c.h*( FENEForces.FENE_x1 - FENEForces.FENE_x2 + BrownianForces.BrownianForce_x)/StokeDragForce));
...
POSITION updateLast(POSITION nMinusOnePos, POSITION nPosOld, POSITION nPosPlusOne, POSITION nPosNew, CONSTANTS c){
    ...
    nPosNew.xPos = nPosOld.xPos + (c.h*( -FENEForces.FENE_x2 + BrownianForces.BrownianForce_x)/StokeDragForce);            //x INCREASES by FENE acc (a = F/m) and Brownian acc

    nPosNew.yPos = nPosOld.yPos + (c.h*( -FENEForces.FENE_y2 + BrownianForces.BrownianForce_y)/StokeDragForce);

    nPosNew.zPos = nPosOld.zPos + (c.h*( -FENEForces.FENE_z2 + BrownianForces.BrownianForce_z)/StokeDragForce);
